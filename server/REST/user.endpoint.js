...
router.post('/api/user/comments', auth, async (req, res, next) => {
  ...
  const userId = req.user.userId; // userId z tokena
  ...
  try {
    const result = await business.getUserManager(req).addComment(userId, movieId, commentText);
...
// Endpoint do pobierania zagnieżdżonych komentarzy (komentarzy głównych + odpowiedzi)
router.get('/api/user/comments/:movieId', async (req, res) => {
  const { movieId } = req.params;
  ...
  try {
    const totalComments = await Comment.countDocuments({ movieId, parentId: null });
    const comments = await Comment.aggregate([
      { $match: { movieId: mongoose.Types.ObjectId(movieId), parentId: null } },
      {
        $lookup: {
          from: 'comments',
          localField: '_id',
